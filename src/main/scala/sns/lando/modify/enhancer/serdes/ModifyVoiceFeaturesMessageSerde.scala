...
class ModifyVoiceFeaturesMessageSerde extends Serde[Transaction] {
  ...
  override def serializer(): Serializer[Transaction] = new ModifyVoiceFeaturesMessageSerializer()
...
class ModifyVoiceFeaturesMessageSerializer extends Serializer[Transaction] {
  ...
  override def serialize(topic: String, voiceFeatures: Transaction): Array[Byte] = voiceFeaturesParser.parse(voiceFeatures).getBytes
...
class TransactionDeserializer extends Deserializer[Transaction] {
  ...
  override def deserialize(topic: String, data: Array[Byte]): Transaction = voiceFeaturesParser.parse(new String(data))
