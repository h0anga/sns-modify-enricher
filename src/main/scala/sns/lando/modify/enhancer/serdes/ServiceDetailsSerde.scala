...
class ServiceDetailsSerde extends Serde[ServiceDetails] {
  ...
  override def serializer(): Serializer[ServiceDetails] = new ServiceDetailsSerializer()

  override def deserializer(): Deserializer[ServiceDetails] = new ServiceDetailsDeserializer()
}
